...
our constant $NORMAL      = "\x0f";

# Text formats
our constant $BOLD        = "\x02";
our constant $UNDERLINE   = "\x1f";
our constant $REVERSE     = "\x16";
our constant $ITALIC      = "\x1d";
our constant $FIXED       = "\x11";
our constant $BLINK       = "\x06";

# Color formats
our constant $WHITE       = "\x0300";
our constant $BLACK       = "\x0301";
our constant $BLUE        = "\x0302";
our constant $GREEN       = "\x0303";
our constant $RED         = "\x0304";
our constant $BROWN       = "\x0305";
our constant $PURPLE      = "\x0306";
our constant $ORANGE      = "\x0307";
our constant $YELLOW      = "\x0308";
our constant $LIGHT_GREEN = "\x0309";
our constant $TEAL        = "\x0310";
our constant $LIGHT_CYAN  = "\x0311";
our constant $LIGHT_BLUE  = "\x0312";
our constant $PINK        = "\x0313";
our constant $GREY        = "\x0314";
our constant $LIGHT_GREY  = "\x0315";

# Associates numeric codes with their string representation
our constant %NUMERIC2NAME =
...
our constant %NAME2NUMERIC = %NUMERIC2NAME.keys.map: {
    %NUMERIC2NAME{$_} => .Int
}

sub numeric_to_name(Int:D $code --> Str:D) is export {
    %NUMERIC2NAME{$code}
}

sub name_to_numeric(Str:D $name --> Int:D) is export {
    %NAME2NUMERIC{$name}
}

sub uc_irc(Str:D $value, Str $type = 'rfc1459' --> Str:D) is export {
    given ($type // '').lc {
        when 'ascii' {
            $value.trans('a..z' => 'A..Z');
        }
...
sub lc_irc(Str:D $value , Str $type = 'rfc1459' --> Str:D) is export {
    given $type.lc {
        when 'ascii' {
            $value.trans('A..Z' => 'a..z');
        }
...
sub eq_irc(Str $first, Str $second, Str $type = 'rfc1459' --> Bool:D) is export {
    $first.defined && $second.defined
      ?? lc_irc($first, $type) eq lc_irc($second, $type)
      !! False
}
        ...
        if $arg.^name eq 'Array' {
            ...
            for $arg.comb -> $c {
                ...
                if @chan_modes[0].elems
                    ...
                    # now illegal in Raku. Imagine this as if it were:
                    #
...
sub normalize_mask(Str:D $mask is copy --> Str:D) is export {
    my @normalized;
    my $remainder;
...
sub unparse_mode_line(Str:D $line --> Str:D) is export {
    return '' unless $line;
    ...
    for $line.comb -> $mode {
        if $mode eq "+" | "-" {
            if !$action.defined || $mode ne $action {
                $return ~= $mode;
                $action  = $mode;
            }
        }
...
--> Str:D) is export {
    ...
    unparse_mode_line($string)
}

sub is_valid_nick_name(Str:D $nick --> Bool:D) is export {
    #my regex complex {  _ \` \- \^ \| \\ \{\} \[\] };
    #my regex complex { '_' '`' '-' '|' '\\' '{' '}' '[' ']' };
...
sub is_valid_chan_name(Str $chan, $types = ['#', '&'] --> Bool:D) is export {
    ...
    for $types -> $t {
        ...
        return False if $c !~~ /^ $t <-[ \s \c07 \c0 \c012 \c015 , :]>+ $/;
    }
...
sub parse_user(Str:D $user --> List:D) is export {
    $user.split(/<[!@]>/).list
}

sub has_color(Str:D $string --> Bool:D) is export {
    so $string ~~ /<[\x03 \x04 \x1b]>/
}

# TODO Create rule/regex for matching format codes to reduce duplication
#      in has_formatting() and strip_formatting()

sub has_formatting(Str:D $string --> Bool:D) is export {
    so $string ~~ /<[\x02 \x1f \x16 \x1d \x11 \x06]>/
}

sub strip_color(Str:D $string is copy --> Str:D) is export {
    $string = $string
    # Strip mIRC colors
      .subst(/\x03 [\, \d**1..2 | \d**1..2 [\, \d**1..2]?]?/, :g)

    # Strip other colors supported by certain clients
      .subst(/:i \x04 <[0..9 a..f A..F]>**0..6/, :g)

    # Strip ANSI escape codes
      .subst(/\x1b \[ .*? <[\x00..\x1f \x40..\x7e]>/, :g);

    # Strip terminating \x0f only if there aren't any formatting codes
    has_formatting($string)
      ?? $string
      !! $string.subst(/\x0f/)
}

sub strip_formatting(Str $string is copy) returns Str is export {
    $string .= subst(/<[\c017 \c02 \c037 \c026 \c035 \c021 \c06]>/, :g);
    #$string .= subst(/<[\x0f \x02 \x1f \x16 \x1d \x11 \x06]>/, :g);
...
my sub _diff(@before, @after --> Array:D) {
    my %in_before;
    my %in_after;
...
=head2 uc_irc(Str $value, Str $type?)
...
=head2 eq_irc(Str $first, Str $second, Str $type?)
...
=head2 numeric_to_name(Int $code)
...
=head2 name_to_numeric(Str $name)
...
=head2 is_valid_nick_name(Str $nick)
...
=head2 is_valid_chan_name(Str $chan, Str @types?)
...
=head2 unparse_mode_line(Str $line)
...
=head2 gen_mode_change(Str $before, Str $after)
...
=head2 matches_mask(Str $mask, Str $match, Str $mapping?)
...
=head2 has_formatting(Str $string)
...
=head2 strip_formatting(Str $string)
